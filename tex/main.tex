...
parameters of the line $(i,j)$ (or to the associated transformer).
...
encoded as a vector $\alpha_g \in \mathbb{R}^{n_g}$, the power generation in contingency $k$ is given by
\begin{equation}
  \label{eq:agc}
...
\label{sec:ncl:ncl}
The augmented Lagrangian method needs a penalty $\rho^{(n)}$
...
As a result, the filter line-search algorithm used internally to solve~\eqref{eq:nclsubpb} has to
