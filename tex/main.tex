...
% in the abstract
\begin{abstract}
  We present a new algorithm for solving large-scale security-constrained optimal power flow (SC-OPF) in polar form. The method builds on Algorithm NCL, an augmented Lagrangian method in which subproblems are solved using an interior-point method. NCL has two key advantages for large-scale SC-OPF. First, it is robust, handling difficult problems such as infeasible ones or models with complementarity constraints. Second, the augmented Lagrangian term naturally regularizes the Newton linear systems within the interior-point method, enabling of a pivoting-free factorization that can be efficiently parallelized on GPs. We assess NCL's performance on large-scale corrective AC-SCOPFs with complementarity constraints modeling the corrective actions. Numerical results show that NCL significantly outperforms the state-of-the-art solver Knitro in solving large-scale SCOPF.
\end{abstract}
...
a security-constrained optimal power flow (SCOPF). The dispatch should
minimize a given criterion (costs or network losses) while considering
...
This comes at the price of linearizing the nonlinear physical constraints, incurring the solution's feasibility~\cite{coffrin2012approximating}.
Solving the AC-SCOPF with the original nonlinear formulation remains an open challenge,
and was the main motivation behind the recent GO-competition recently orchestrated by the FERC~\cite{aravena2023recent}.
...
like Ipopt or Knitro~\cite{capitanescu2011state}. Second, when incorporating recourses, the AC-SCOPF considers a set of logical conditions to encode the
droop control and the PV/PQ switches in the post-contingency state.
...
on in power system computation, be it for power flow~\cite{kim2022mixed,zhao2008pv,sundaresh2014modified,murray2015robust} or for optimal power flow~\cite{rosehart2005optimal}.
...
However, the AC-SCOPF reformulated with complementarity constraints yields degenerate nonlinear program
with its own issues.

\subsection{Augmented Lagrangian method}

There exists numerous algorithms to solve mathematical programs with complementarity constraints (MPCC).
For instance, the teams in the GO-competition has handled the complementarity constraints
using active-set methods~\cite{curtis2023decomposition}, homotopy reformulations~\cite{gholami2023admm,petra2023surrogate} or a $\ell_1$-exact penalty reformulation (the solver Knitro used during the competition implements
the method described in~\cite{leyffer2006interior}).
...
To address this challenge, we propose instead to solve the SCOPF with an Augmented Lagrangian
method based on the Algorithm NCL~\cite{ma2017stabilized,montoison2025madncl}. The augmented Lagrangian comes with three benefits.
...
In this work, we analyze the performance of MadNCL on large-scale AC-SCOPF instances.
MadNCL is a new implementation
of the augmented Lagrangian method based on Algorithm NCL. In particular,
\begin{itemize}
  \item We present how we can perform the contingency screening and the solution
    of the AC-SCOPF with MadNCL.
...
$(v_m^k, v_a^k)$, and the active and reactive power generation by $(p_g^k, q_g^k)$.
...
In SCOPF, the variation of the power production $p_g^k$ in contingency $k$
...
and $\pi_{g,-}^k \geq 0$, equation~\eqref{eq:agc} rewrites, for
all $k=1, \cdots, K$,
\begin{equation}
  \label{eq:droopmpec}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation}
Similarly, the \emph{voltage control} keep the voltage
...
its upper or lower limit, the bus should be converted to a PQ bus and the voltage allowed
to vary. This additional constraint is modeled with a second set of complementarity constraints writing,
for $\nu_-^k \geq 0$ and $\nu_+^k \geq 0$,
\begin{equation}
  \label{eq:pvpq}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation}
As a result, the corrective SCOPF writes as a nonlinear program with complementarity
...
We note $u_0 = (p_g^0, v_{m,pv}^0)$ and $x_0 = (v_a^0, v_{m,pq}^0)$ the control and the state in the base case,
and $u_k = (p_g^k, v_{m,pv}^k)$ and $x^k = (v_a^k, v_{m,pq}^k, \Delta^k, \pi_g^k, \nu^k)$
...
penalized in the objective to ensure feasibility, as this will be handled automatically by Algorithm NCL.
Note that the only coupling between the base case and the contingencies
appears in the complementarity conditions.

For a given control $u_0$, the contingency $k$ is feasible if there is a solution
...
\begin{equation}
  ...
  \right.
\end{equation}
The system \eqref{eq:screening} allows to determine whether the contingency $k$ is feasible.
...
does not satisfy any constraint qualification, rendering its direct solution by a classical
solver difficult.
...
In the next section, we interpret the algorithm NCL as a mix of the regularization and penalty-based methods.
...
and are easy to solve with a projected gradient or Newton methods.
Instead, NCL solves the nonlinearly constrained subproblem at the outer iteration $n$:
\begin{equation}
  \label{eq:nclsubpb}
  \begin{aligned}
...
The subproblem~\eqref{eq:nclsubpb} is always feasible. We present in the two subsequent paragraphs the inner workings of NCL.
...
with an interior-point method, up to a given tolerance $\omega^{(n)}$.
Only the objective changes between two successive outer iterations, hence the algorithm used to solve
...
we can quickly assess which contingency and constraints are rendering the SCOPF problem~\eqref{eq:scopf} infeasible.
...
Lagrangian algorithm is penalizing the violation directly for us.
...
solves the system~\eqref{eq:kktncl} for $(w, r, t, \lambda, \nu)$ iteratively using a homotopy method, replacing
the complementarity constraints by
\begin{equation}
  S V_0 = \mu e \; , \; W_1 V_1 = \mu e \; , \; W_2 V_2 = \mu e \; ,
\end{equation}
with $S = \text{diag}(s)$, $V_i = \text{diag}(\nu_i)$ for $i=1,2,3$,
and $\mu > 0$ a given barrier parameter.

Our implementation of NCL uses MadNLP to solve \eqref{eq:kktncl}. The barrier is updated
using the Fiacco-McCormick rule. The globalization is performed using a filter line-search
algorithm. At each IPM iteration, the Newton system associated to NCL subproblem reduces to
\begin{equation}
...
\begin{equation}
  ...
  \end{bmatrix} \; ,
\end{equation}
and the Jacobian and the regularization term:
\begin{equation}
  \label{eq:blockBC}
  B = \begin{bmatrix}
    J_0 & J_1 & J_2 \\
...
$(2,2)$ block $C$ is non-zero in \eqref{eq:newtonsystem}: as it is well known, the augmented Lagrangian
term adds a natural dual regularization to the system. This regularization
is responsible for NCL's robustness on degenerate instances.
...
In that case, the non-zero block $C$ is determinant to ensure \eqref{eq:newtonsystem} is well posed.
...
in Ipopt and MadNLP to ensure the reduced Hessian of \eqref{eq:newtonsystem} is positive definite.
The problem is amplified if $\nu_0$ converges to large value. Thankfully, we have
...
As a result, traditional solvers often fallback to HSL MA27 or MA57 to solve \eqref{eq:newtonsystem}.
...
The algorithm NCL has been implemented in Julia 1.11.
Our implementation, called MadNCL, uses the solver MadNLP to solve
the subproblems~\eqref{eq:nclsubpb}. As a consequence, MadNCL is able to run
either on the CPU or on the GPU.
The implementation is described in length in \cite{montoison2025madncl}.
The implementation is available at this URL: \url{https://github.com/MadNLP/MadNCL.jl}.
...
a modeler that supports the evaluation of the derivatives in parallel. We found that
ExaModels is at least 10x faster than JuMP on the CPU. Furthermore, ExaModels supports
the evaluation of the derivatives fully on the GPU, which gives us an additional speed-up factor.
When MadNCL runs on the CPU, the Newton systems are solved with HSL MA57.
...
Screening of contingencies is an important part in the pre-processing~\cite{capitanescu2007contingency}.
...
In that case, NCL falls back to the feasibility problem~\eqref{eq:feaspb} to return the
regularization $r$ that minimizes the constraint deviation: the binding constraints
can be identified in the post-processing.
...
The final objective $\|r^k\|^2+ \|t^k\|^2$ quantifies the contingency's infeasibility: the closer to 0,
the more feasible it is. On the contrary, a large objective indicate that the contingency is infeasible.
