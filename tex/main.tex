...
like Ipopt or Knitro~\cite{capitanescu2011state}. Second, when incorporating recourse in the post-contingency state, the AC-SCOPF has to encode a set of logical conditions modeling the
...
\begin{itemize}
  ...
  \item We detail the GPU implementation of MadNCL, from the evaluation
...
and the contingency $k$, here expressed with the complementarity constraints
\eqref{eq:droopmpec} and \eqref{eq:pvpq}.
On the contrary to \cite{curtis2023decomposition}, we do relax the problem
to ensure feasibility, as this will be handled latter by Algorithm NCL.
...
At each outer iteration $n$, NCL solves the nonlinearly constrained subproblem:
\begin{equation}
  \label{eq:nclsubpb}
  \begin{aligned}
...
As a result, traditional optimization solvers often resort to HSL MA27 or MA57 to solve \eqref{eq:newtonsystem}.
...
reformulates the problem in Scholtes form~\eqref{eq:mpccnlp}. We observe that in term
...
of derivatives (in particular on the GPU). MadNCL solves the Scholtes reformulation~\eqref{eq:mpccnlp}.
