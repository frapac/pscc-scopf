...
\begin{equation}
  \begin{aligned}
    & \sum_{g \in \mathcal{G}(i)} p_g^k - p_{\ell,i} - g_i^{sh} v_i^2 - \sum_{j \in \mathcal{N}(i)} p_{ij} = 0 \; , \\
    & \sum_{g \in \mathcal{G}(i)} q_g^k - q_{\ell,i} + b_i^{sh} v_i^2 - \sum_{j \in \mathcal{N}(i)} q_{ij} = 0 \; ,\\
    & \text{with} \\
    & p_{ij} = g_{ii} v_i^2 - (g_{ij} \cos(\theta_i - \theta_j) + b_{ij} \sin(\theta_i - \theta_j))v_i v_j   \\
    & q_{ij} = -b_{ii} v_i^2 + (b_{ij} \cos(\theta_i - \theta_j) - g_{ij} \sin(\theta_i - \theta_j)) v_i v_j  \\
  \end{aligned}
\end{equation}
The line current rating constraints are:
\begin{equation}
  p_{ij}^2 + q_{ij}^2 \leq \overline{\ell}_{ij} \; , \;
  p_{ji}^2 + q_{ji}^2 \leq \overline{\ell}_{ij} \; .
\end{equation}
...
of this paper. We also thank the three anonymous reviewers for their meaningful remarks,
