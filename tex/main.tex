...
\begin{document}
\title{
An accelerated Augmented Lagrangian method for Security-Constrained Optimal Power Flow
}
...
% in the abstract
\begin{abstract}
The Latex template and basic guidelines for the preparation of a technical paper for the PSCC 2026 conference are presented. The abstract is limited to 150 words and cannot contain equations, figures, tables, or references. It should concisely state what was done, how it was done, principal results, and their significance.
\end{abstract}
...
Furthermore, the dispatch should satisfy the $N-1$ criterion, meaning it should remain feasible
for a set of contingency scenarios corresponding to the loss of a line or a generator in the network.

The SCOPF is usually formulated as a large-scale linear program, whose size
grows linearly with the number of contingencies.
This comes at the price of linearizing the physical constraints, incurring the solution's feasibility~\cite{coffrin2012approximating}.
Solving the SCOPF with the original nonlinear formulation remains an open challenge,
and was the main motivation of the recent GO-competition recently orchestrated by the FERC~\cite{aravena2023recent}.
The issue with the nonlinear formulation is two-fold. First, the SCOPF writes as a huge-scale
nonlinear programs, whose size is beyond the capacity of state-of-the-art nonlinear optimization solvers
like Ipopt or Knitro~\cite{capitanescu2011state}. Second, the SCOPF considers a set of logical conditions to encode the
...
easier to handle in the large-scale regime. This second method has been adopted by some
of the leading teams during the GO-competition, as it integrates neatly with interior-point solvers
\cite{curtis2023decomposition,gholami2023admm}.
However, the SCOPF reformulated with complementarity constraints yields degenerate nonlinear program
with its own issues.

\subsection{Augmented Lagrangian method}

There exists numerous algorithms to solve mathematical programs with complementarity constraints (MPCC).
More particularly, the teams in the GO-competition has handled the complementarity constraints
using an active-set method~\cite{curtis2023decomposition}, a homotopy reformulation~\cite{gholami2023admm} or a $\ell_1$-exact penalty reformulation (the solver Knitro used during the competition implements
...
method based on the Algorithm NCL~\cite{ma2017stabilized}. The augmented Lagrangian comes with three benefits.
(1) It can quickly detect infeasible problems (a common situation for SCOPF)~\cite{chiche2016augmented}.
...
in the algorithm can be solved efficiently in parallel.
As such, MadNCL can be considered as an extension of our previous work investigating
...
In this work, we analyze the performance of MadNCL on large-scale SCOPF instances.
...
\section{Model}
We start by presenting the SCOPF model. We formulate the SCOPF in polar form,
by explicitly taking into account the nonlinearities in the model.
...
\end{equation}
where $\Delta^k \in \mathbb{R}$ is an optimization variable.
The non-smooth $\min$ and $\max$ operations rewrite equivalently as
a set of complementarity constraints: for $\pi_{g,+}^k \geq 0$
...
to vary. This additional constraint is modeled with a second set of MPEC constraints writing,
for $\nu_-^k \geq 0$ and $\nu_+^k \geq 0$,
\begin{equation}
  \label{eq:pvpq}
  \begin{aligned}
...
constraints. We note that the MPEC formulation has been adopted throughout the GO
...
the control and the state in the contingency $k$.
\begin{equation}
  \label{eq:scopf}
  \begin{aligned}
    \min_{u_0, x_0, u_k, x_k} \; & f(x_0, u_0) \\
    \text{s.t.} \quad & g_0(x_0, u_0) = 0 \; , \;  h_0(x_0, u_0) \leq 0 \; , \\
                            & g_k(x_k, u_k) = 0 \;, \;  h_k(x_k, u_k) \leq 0 &\forall k=1, \cdots, K\\
                            & 0 \leq G(u_0, x_k, u_k) \perp H(x_k, u_k) \geq 0 & \forall k = 1,\cdots, K
  \end{aligned}
\end{equation}
where $g(\cdot)$ encodes the power-flow constraints, $h(\cdot)$ the operational
...
penalized in the objective to ensure feasibility, as this is handled automatically
...
By introducing slack variables to reformulate the inequality constraints
...
\end{equation}
Unfortunately, the problem~\eqref{eq:mpccnlp} is degenerate, in the sense that
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
Every $w \in \Omega$ satisfying strong stationarity satisfies Bouligand stationarity.
We say that MPCC-LICQ holds at $x \in \Omega$ if the multipliers $(\lambda, \mu)$ satisfying
\eqref{eq:strongstationarity} are unique. We note that the conditions
\eqref{eq:strongstationarity} encodes the KKT stationary solution of a regular
nonlinear program.
...
is reformulated as $W_1 W_2 e \leq t$. Such relaxation is called a Scholtes
...
In this section, we interpret the algorithm NCL as a mix of the regularization and penalty-based methods.

\section{Augmented Lagrangian algorithm}
In this section, we present the Augmented Lagrangian algorithm we
are using to solve
Algorithm NCL solves the problem~\eqref{eq:mpccnlp} using an Augmented Lagrangian method.
Apart of the different subproblem formulation \eqref{eq:nclsubpb}, NCL
is strictly equivalent to the classical Augmented Lagrangian method.
In particular, NCL inherits from the all properties of the Augmented Lagrangian method:
the method is able to quickly detect infeasibility.
...
The Augmented Lagrangian solves a nonlinear program iteratively by solving a sequence of
subproblems easier to solve (with only bound constraints or complementarity constraints).
...
with
\begin{equation}
  \mathcal{L}_\rho(w, r, t, \lambda^n, \nu^n_0) = \phi(w) + (\lambda^n)^\top r
  + (\nu_0^n)^\top t + \\ \frac{\rho}{2}(\|r \|^2 + \|t\|^2) \; ,
\end{equation}
