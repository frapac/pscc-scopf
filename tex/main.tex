...
\end{multline}
Problem~\eqref{eq:mpccnlp} is degenerate, in the sense that
...
For solving MPCC~\eqref{eq:mpcc}, it has been proven in \cite{izmailov2012global} that the augmented Lagrangian
...
NCL operates at two level: it updates a penalty $\rho^{(n)}$
...
solve the constrained nonlinear subproblem
\begin{equation}
  \label{eq:nclsubpb}
  \begin{aligned}
...
Subproblem~\eqref{eq:nclsubpb} is always feasible.
...
At a given outer iteration $n$, NCL solves Subproblem \eqref{eq:nclsubpb}
...
have to penalize the constraint violation in the objective using an $\ell_1$ penalty, as the augmented
...
Linear system~\eqref{eq:newtonsystem} is sparse symmetric indefinite.
Hence, traditional optimization solvers often apply HSL~\cite{duff1983multifrontal}.
...
However, the non-zero block $C$ adds a regularization to the system, and this allows
solution of \eqref{eq:newtonsystem} with a pivoting-free factorization, e.g., using the signed Cholesky factorization~\cite{montoison2025madncl}.
