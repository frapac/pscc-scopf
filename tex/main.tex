...
coming with its own set of issues.
...
$(v_b^k, \theta_b^k) \in \mathbb{R}^{n_b} \times \mathbb{R}^{n_b}$,
\add{the loads by $(p_{\ell}, q_\ell) \in \mathbb{R}^{n_b} \times \mathbb{R}^{n_b}$ and the active and reactive power generations by $(p_g^k, q_g^k) \in \mathbb{R}^{n_g} \times \mathbb{R}^{n_g}$.
The set of arcs in the network is denoted by $\mathcal{A}$.}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
\add{The active and reactive power flow constraints at each bus are,
for $i=1, \cdots, n_b$:
\begin{equation*}
  \begin{aligned}
    ...
    & \text{with, for all arcs $(i, j) \in \mathcal{A}$}, \\
...
$\mathcal{G}(i)$ (resp. $\mathcal{N}(i)$) the set of generators (resp. buses) connected to $i$,
and $(y_{1ij}, y_{2ij}, y_{3ij}, y_{4ij})$ four coefficients associated to the physical
parameters of the line $(i,j)$. The line current rating constraints are:
\begin{equation*}
...
line has an outage in contingency $k$).}
...
encoded by a vector $\alpha_g \in \mathbb{R}^{n_g}$, the power generation in contingency $k$ is given by
\begin{equation}
  \label{eq:agc}
...
a set of complementarity constraints~\cite{baumrucker2008mpec}: for non-negative $(\pi_{g+}^k, \pi_{g-}^k) \geq 0$, Equation~\eqref{eq:agc} is equivalent to, for all $k=1, \cdots, K$,
\begin{equation}
  \label{eq:droopmpec}
  \begin{aligned}
...
and $u_k = (p_g^k, v_{b,PV}^k)$, $x^k = (\theta_b^k, v_{b,PQ}^k, q_g^k, \Delta^k, \pi_g^k, \nu^k_b)$
...
\add{In contrast to the formulation used by the GO-competition~\cite{aravena2023recent}, we do not
...
\add{The complementarity constraints complicate the solution of the problem \eqref{eq:scopf}.
As such, the complementarity constraints are usually reformulated with binary variables and a big-M reformulation,
...
Instead, we choose here to address directly the complementarity constraints
and solve \eqref{eq:scopf} instead using an augmented Lagrangian method based on the algorithm NCL.}
...
and $c: \mathbb{R}^n \to \mathbb{R}^m$ a function aggregating the constraints in \eqref{eq:scopf}.
...
\end{multline}
\add{The complementarity constraints in \eqref{eq:mpcc} state that for $i=1,\cdots,p$, $w_{1i} w_{2i} = 0$
and $(w_{1i}, w_{2i}) \geq 0$. As both variables in the product are positive,
the complementarity constraints rewrite equivalently as $w_{1i} w_{2i} \leq 0$
and $(w_{1i}, w_{2i}) \geq 0$: this alternative formulation
...
\end{multline}
\add{Problem~\eqref{eq:mpccnlp} is degenerate and the Mangasarian-Fromovitz constraint qualification (MFCQ)
fails for all feasible points. Indeed, suppose we have a single complementarity
constraint with $(w_1, w_2) \geq 0$ and $w_1 w_2 \leq 0$. By definition, a feasible point $(w_1, w_2)$ satisfies
...
which is a contradiction as $w_2$ is positive. Same reasoning applies if $w_2 = 0$ and $w_1 > 0$.
If the complementarity constraint
is biactive (both $w_1 = 0$ and $w_2 = 0$) the condition $w_2 d_1 + w_1 d_2 < 0$ cannot hold.
Hence MFCQ is not satisfied, meaning the set of optimal multipliers $(\lambda, \nu)$ can be unbounded,
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
\add{As for any $w \in \Omega$, $(w_1, w_2) \geq 0$ the sets \eqref{eq:indexsets} define
a partition of $\{1, \cdots, p\}$.}
...
Our goal is to find a strong stationary solution \eqref{eq:strongstationarity} for \eqref{eq:mpcc}.
...
\add{We emphasize that NCL treats \eqref{eq:mpccnlp} as a generic nonlinear program, and does not
apply any special treatment to the complementarity constraints.}

\subsection{Algorithm NCL}
\label{sec:ncl:ncl}
\add{Let a penalty $\rho^{(n)}$
...
the augmented Lagrangian subproblem is defined as:
\begin{equation}
...
and even when it is defined, the Hessian $\nabla_{x x}^2 \mathcal{L}_\rho(\cdot)$ gets ill-conditioned as we increase the penalty $\rho$.}

NCL is strictly equivalent to the classical augmented Lagrangian method,
but uses a nonlinearly constrained formulation in the subproblems~\eqref{eq:auglagsubpb}.
For two free regularization variables $(r, t) \in \mathbb{R}^m \times \mathbb{R}^p$, NCL solves:
\begin{equation}
  \label{eq:nclsubpb}
  \begin{aligned}
  ...
  \end{aligned}
\end{equation}
\add{The regularization $t$ plays the role of the Scholtes relaxation parameter (see \S\ref{sec:mpcc:solution})}
and is penalized in the objective by the Augmented Lagrangian penalty.

\add{We make a distinction between the {outer} and the {inner} iterations.
...
\add{For a given forcing sequence $\{\eta^{(n)}\}_n$, NCL uses the classical updates:
\begin{itemize}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
\add{As discussed in \cite{izmailov2012global}, the augmented Lagrangian algorithm
...
If this condition is satisfied, the solution of the linear system~\eqref{eq:newtonsystem} is a valid descent direction for the filter. The reduced Hessian is positive definite if and only if the inertia
of the system~\eqref{eq:newtonsystem} is $(n, m+p, 0)$.
If the inertia is not correct, MadNLP adds a positive term on the diagonal
entries of the matrix $A$ and increases it until the inertia becomes exactly equal to $(n, m+p, 0)$.
This procedure requires computing multiple matrix factorization, and can be very costly.

For that reason, the linear system~\eqref{eq:newtonsystem} is usually factorized using an inertia-revealing
linear solver. The Duff-Reid algorithm~\cite{duff1983multifrontal}, as implemented in the HSL solvers,
decomposes the system~\eqref{eq:newtonsystem} as
...
the inertia of the matrix can be directly deduced from the blocks in $B$.
...
the subproblems~\eqref{eq:nclsubpb}. We can run MadNLP either on the CPU or on the GPU \cite{shin2024accelerating}.
...
ExaModels supports evaluation of the derivatives on the GPU, which gives us an additional speed-up.
The Newton systems~\eqref{eq:newtonsystem} are solved with HSL MA57 on the CPU, or on
the GPU using the linear solver NVIDIA cuDSS (implementing the signed Cholesky factorization
described in \S\ref{sec:algo:linsolvers}).

We compare MadNCL with the interior-point solver Knitro~\cite{waltz2006interior},
running on the CPU also with the linear solver HSL MA57. Knitro evaluates the SCOPF model~\eqref{eq:scopf}
...
We use instances from the MATPOWER library~\cite{zimmerman2010matpower} and
\add{from the Challenge 1 of the GO competition}\footnote{\url{https://data.openei.org/submissions/6153}}.
...
In all experiments, we set the convergence tolerance to {\tt tol=1e-5}.
...
regularization variables $(r, t)$ minimizing the local infeasibility (with guarantees in the convex case~\cite{chiche2016augmented}).
...
% \begin{figure}[!ht]
% \centering
% \includegraphics[width=.3\textwidth]{./figures/contingency.pdf}
...
MadNCL solves the NLP reformulation~\eqref{eq:mpccnlp} and uses
the modeler ExaModels~\cite{shin2024accelerating}, which can evaluate the model both on the CPU or on the GPU.
...
We detail the results in Table~\ref{tab:scopfscalability}. \add{In addition to the performance we obtain with ExaModels on the CPU and on the GPU,
we add two columns detailing the running time we obtain with MadNCL on the CPU using the default
automatic differentiation in JuMP ({\tt SparseReverseMode}) and with symbolic automatic differentiation
({\tt SymbolicAD}).}
Interestingly, the number of iterations in MadNCL depends on the modeler and on the linear solver: as we enter the large-scale
...
to different solutions. However, we observe that MadNCL converges to the same solution as Knitro.
We show in Figure~\ref{fig:scalability} the time per iteration (in seconds) against
the number of contingencies for Knitro and MadNCL \add{(running on the CPU with JuMP, SymbolicAD and ExaModels
and on the GPU with ExaModels)}.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=.4\textwidth]{./scalability.pdf}
\includegraphics[width=.4\textwidth]{./scalability_decomposed.pdf}
\caption{Total time, time in linear solver and time in AD reported per iteration (in seconds), against number of contingencies $K$
  when solving {\tt ACTIVSg500}. \add{We compare Knitro (using JuMP)
  against MadNCL (using JuMP, SymbolicAD and ExaModels)}.
We test MadNCL both on the CPU (using HSL MA57) and on the GPU (using NVIDIA cuDSS).}
...
Second, we compare Knitro and MadNCL-GPU on different instances, with a varying number of contingencies $K$ and
with a time limit set to 1h.
The results are displayed in Table~\ref{tab:benchmark}.
\add{We use the contingency screening described in \S\ref{sec:numerics:screening} to remove
the structurally infeasible contingencies.
Apart from the contingency screening,
our algorithm solves the problem \eqref{eq:scopf} frontally without using any heuristic or structure exploiting method to speed-up the solution time.}
On this new benchmark, we observe that MadNCL-GPU is consistently faster than Knitro. As reported in Table~\ref{tab:scopfscalability},
...
As a result, the filter line-search algorithm used internally to solve~\eqref{eq:nclsubpb}
performs numerous primal-dual regularizations during the inertia correction, significantly impairing MadNCL's convergence (we have observed that the ill-conditioning arises mostly from the reformulation
...
MadNCL can find in less than 3 minutes a strongly stationary solution for AC-SCOPFs with almost a million decision variables, allowing instances with thousands of buses and
...
\bibitem{stott2005security}
B.~Stott, O.~Alsac, and A.~J. Monticelli, ``Security analysis and optimization,'' \emph{Proceedings of the IEEE}, vol.~75, no.~12, pp. 1623--1644, 1987.

\bibitem{frank2016introduction}
...
\bibitem{alsac2002further}
O.~Alsac, J.~Bright, M.~Prais, and B.~Stott, ``Further developments in {LP}-based optimal power flow,'' \emph{IEEE Transactions on Power Systems}, vol.~5, no.~3, pp. 697--711, 2002.

\bibitem{coffrin2012approximating}
C.~Coffrin, P.~Van~Hentenryck, and R.~Bent, ``Approximating line losses and apparent power in {AC} power flow linearizations,'' in \emph{2012 IEEE power and energy society general meeting}.\hskip 1em plus 0.5em minus 0.4em\relax IEEE, 2012, pp. 1--8.

\bibitem{aravena2023recent}
I.~Aravena, D.~K. Molzahn, S.~Zhang, C.~G. Petra, F.~E. Curtis, S.~Tu, A.~W{\"a}chter, E.~Wei, E.~Wong, A.~Gholami \emph{et~al.}, ``Recent developments in security-constrained {AC} optimal power flow: Overview of challenge 1 in the {ARPA-E} grid optimization competition,'' \emph{Operations research}, vol.~71, no.~6, pp. 1997--2014, 2023.

\bibitem{wachter2006implementation}
A.~W{\"a}chter and L.~T. Biegler, ``On the implementation of an interior-point filter line-search algorithm for large-scale nonlinear programming,'' \emph{Mathematical programming}, vol. 106, no.~1, pp. 25--57, 2006.

\bibitem{waltz2006interior}
R.~A. Waltz, J.~L. Morales, J.~Nocedal, and D.~Orban, ``An interior algorithm for nonlinear optimization that combines line search and trust region steps,'' \emph{Mathematical programming}, vol. 107, no.~3, pp. 391--408, 2006.

\bibitem{capitanescu2011state}
F.~Capitanescu, J.~M. Ramos, P.~Panciatici, D.~Kirschen, A.~M. Marcolini, L.~Platbrood, and L.~Wehenkel, ``State-of-the-art, challenges, and future trends in security constrained optimal power flow,'' \emph{Electric power systems research}, vol.~81, no.~8, pp. 1731--1741, 2011.

\bibitem{zhao2008pv}
J.~Zhao, H.-D. Chiang, H.~Li, and P.~Ju, ``On {PV-PQ} bus type switching logic in power flow computation,'' in \emph{Power Systems Computation Conference (PSCC), Glasgow, Scotland}, 2008.

\bibitem{sundaresh2014modified}
L.~Sundaresh and P.~N. Rao, ``A modified {N}ewton--{R}aphson load flow scheme for directly including generator reactive power limits using complementarity framework,'' \emph{Electric power systems research}, vol. 109, pp. 45--53, 2014.

\bibitem{murray2015robust}
W.~Murray, T.~Tinoco De~Rubira, and A.~Wigington, ``A robust and informative method for solving large-scale power flow problems,'' \emph{Computational optimization and applications}, vol.~62, no.~2, pp. 431--475, 2015.

\bibitem{rosehart2005optimal}
W.~Rosehart, C.~Roman, and A.~Schellenberg, ``Optimal power flow with complementarity constraints,'' \emph{IEEE Transactions on Power Systems}, vol.~20, no.~2, pp. 813--822, 2005.

\bibitem{nurkanovic2024}
\BIBentryALTinterwordspacing
A.~Nurkanović, A.~Pozharskiy, and M.~Diehl, ``Solving mathematical programs with complementarity constraints arising in nonsmooth optimal control,'' \emph{Vietnam Journal of Mathematics}, vol.~53, no.~3, p. 659–697, Aug. 2024. [Online]. Available: \url{http://dx.doi.org/10.1007/s10013-024-00704-z}
\BIBentrySTDinterwordspacing

\bibitem{curtis2023decomposition}
F.~E. Curtis, D.~K. Molzahn, S.~Tu, A.~W{\"a}chter, E.~Wei, and E.~Wong, ``A decomposition algorithm with fast identification of critical contingencies for large-scale security-constrained {AC-OPF},'' \emph{Operations Research}, vol.~71, no.~6, pp. 2031--2044, 2023.

\bibitem{gholami2023admm}
A.~Gholami, K.~Sun, S.~Zhang, and X.~A. Sun, ``An {ADMM}-based distributed optimization method for solving security-constrained alternating current optimal power flow,'' \emph{Operations Research}, vol.~71, no.~6, pp. 2045--2060, 2023.

\bibitem{petra2023surrogate}
C.~G. Petra and I.~Aravena, ``A surrogate-based asynchronous decomposition technique for realistic security-constrained optimal power flow problems,'' \emph{Operations Research}, vol.~71, no.~6, pp. 2015--2030, 2023.

\bibitem{leyffer2006interior}
S.~Leyffer, G.~L{\'o}pez-Calva, and J.~Nocedal, ``Interior methods for mathematical programs with complementarity constraints,'' \emph{SIAM Journal on Optimization}, vol.~17, no.~1, pp. 52--77, 2006.

\bibitem{ma2017stabilized}
D.~Ma, K.~L. Judd, D.~Orban, and M.~A. Saunders, ``Stabilized optimization via an {NCL} algorithm,'' in \emph{Numerical Analysis and Optimization}.\hskip 1em plus 0.5em minus 0.4em\relax Springer, 2017, pp. 173--191.

\bibitem{montoison2025madncl}
A.~Montoison, F.~Pacaud, M.~Saunders, S.~Shin, and D.~Orban, ``{MadNCL}: A {GPU} implementation of algorithm {NCL} for large-scale, degenerate nonlinear programs,'' \emph{arXiv preprint arXiv:2510.05885}, 2025.

\bibitem{chiche2016augmented}
A.~Chiche and J.~C. Gilbert, ``How the augmented {L}agrangian algorithm can deal with an infeasible convex quadratic optimization problem,'' \emph{Journal of Convex Analysis}, vol.~23, no.~2, 2016.

\bibitem{izmailov2012global}
A.~F. Izmailov, M.~V. Solodov, and E.~Uskov, ``Global convergence of augmented {L}agrangian methods applied to optimization problems with degenerate constraints, including problems with complementarity constraints,'' \emph{SIAM Journal on Optimization}, vol.~22, no.~4, pp. 1579--1606, 2012.

\bibitem{shin2024accelerating}
S.~Shin, M.~Anitescu, and F.~Pacaud, ``Accelerating optimal power flow with {GPUs}: {SIMD} abstraction of nonlinear programs and condensed-space interior-point methods,'' \emph{Electric Power Systems Research}, vol. 236, p. 110651, 2024.

\bibitem{baumrucker2008mpec}
B.~Baumrucker, J.~G. Renfro, and L.~T. Biegler, ``{MPEC} problem formulations and solution strategies with chemical engineering applications,'' \emph{Computers \& Chemical Engineering}, vol.~32, no.~12, pp. 2903--2913, 2008.

\bibitem{scheel2000mathematical}
H.~Scheel and S.~Scholtes, ``Mathematical programs with complementarity constraints: Stationarity, optimality, and sensitivity,'' \emph{Mathematics of Operations Research}, vol.~25, no.~1, pp. 1--22, 2000.

\bibitem{fletcher2006local}
R.~Fletcher, S.~Leyffer, D.~Ralph, and S.~Scholtes, ``Local convergence of {SQP} methods for mathematical programs with equilibrium constraints,'' \emph{SIAM Journal on Optimization}, vol.~17, no.~1, pp. 259--286, 2006.

\bibitem{scholtes2001convergence}
S.~Scholtes, ``Convergence properties of a regularization scheme for mathematical programs with complementarity constraints,'' \emph{SIAM Journal on Optimization}, vol.~11, no.~4, pp. 918--936, 2001.

\bibitem{ralph2004some}
D.~Ralph and S.~J. Wright, ``Some properties of regularization and penalization schemes for {MPECs},'' \emph{Optimization Methods and Software}, vol.~19, no.~5, pp. 527--556, 2004.

\bibitem{demiguel2005two}
V.~DeMiguel, M.~P. Friedlander, F.~J. Nogales, and S.~Scholtes, ``A two-sided relaxation scheme for mathematical programs with equilibrium constraints,'' \emph{SIAM Journal on Optimization}, vol.~16, no.~2, pp. 587--609, 2005.

\bibitem{duff1983multifrontal}
I.~S. Duff and J.~K. Reid, ``The multifrontal solution of indefinite sparse symmetric linear,'' \emph{ACM Transactions on Mathematical Software (TOMS)}, vol.~9, no.~3, pp. 302--325, 1983.
...
\bibitem{capitanescu2007contingency}
F.~Capitanescu, M.~Glavic, D.~Ernst, and L.~Wehenkel, ``Contingency filtering techniques for preventive security-constrained optimal power flow,'' \emph{IEEE Transactions on Power Systems}, vol.~22, no.~4, pp. 1690--1697, 2007.
