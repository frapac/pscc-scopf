...
like Ipopt or Knitro~\cite{wachter2006implementation,waltz2006interior,capitanescu2011state}. Second, when incorporating recourse in the post-contingency state, the AC-SCOPF has to encode a set of logical conditions modeling the
...
\begin{itemize}
  \item We present how MadNCL can perform the contingency screening and the solution
    of the corrective AC-SCOPF.
    To the best of our knowledge,
    this is the first time an augmented Lagrangian-based algorithm is used to solve the corrective SCOPF formulated with complementarity constraints.
  \item We show that MadNCL can accommodate well the complementarity constraints in the problem.
    The algorithm is also effective at detecting infeasible problems, a useful feature for contingency screening.
  \item We detail the implementation of MadNCL in Julia. On the GPU, MadNCL evaluates
    the derivatives with ExaModels~\cite{shin2024accelerating} and solves the Newton systems with NVIDIA cuDSS.
...
its lower $\underline{q}_g$ or upper limit $\overline{q}_g$, the bus is converted to a PQ bus and the voltage allowed
...
$g_k(\cdot)$ encodes the power-flow constraints, $h_k(\cdot)$ the operational
...
and the contingency $k$, here expressed with the complementarity parts in
\eqref{eq:droopmpec} and \eqref{eq:pvpq}.

Note that the only coupling between the base case and the contingencies
appears through the complementarity conditions~\eqref{eq:droopmpec}-\eqref{eq:pvpq}.
...
\eqref{eq:strongstationarity} encodes the KKT stationary solution of a relaxed nonlinear program~\cite{scheel2000mathematical}.
We say that MPCC-LICQ holds at $x \in \Omega$ if this regular nonlinear program itself satisfies LICQ.
...
and the two multiplier estimates $(\{\lambda^{(n)}, \nu_0^{(n)}) \in \mathbb{R}^m \times \mathbb{R}^p$ in the \emph{outer iterations}, whereas the \emph{innner iterations}
solves the nonlinear program associated to NCL's subproblem:
\begin{equation}
  \label{eq:nclsubpb}
  \begin{aligned}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
with $(r, t) \in \mathbb{R}^m \times \mathbb{R}^p$ two regularization variables.
...
and proceed to the next outer iteration. For a given forcing sequence $\{\eta^{(n)}\}_n$, NCL uses the classical updates:
\begin{itemize}
...
barrier parameter $\mu > 0$, the complementarity constraints in \eqref{eq:kktncl} are reformulated as
\begin{equation}
...
The impact of this second degeneracy remains limited
if $\nu_0$ converges to $0$, which often happens in practice.
...
and is available at this URL: \url{github.com/MadNLP/MadNCL.jl}.
...
We found that ExaModels is at least 10x faster than JuMP on the CPU. Furthermore, ExaModels supports
...
of computation time, MadNCL is 10 times faster than Knitro. MadNCL is able to detect
infeasible problems (a common occurence in contingency screening) much faster than
...
\begin{table}[!ht]
\centering
\caption{Average time to scan a contingency (in seconds) for different instances.}
\begin{tabular}{|r|rr|}
  \hline
