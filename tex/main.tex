...
the loads by $(p_{\ell}, q_\ell) \in \mathbb{R}^{n_b} \times \mathbb{R}^{n_b}$ and the active and reactive power generations by $(p_g^k, q_g^k) \in \mathbb{R}^{n_g} \times \mathbb{R}^{n_g}$.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
As for any $w \in \Omega$, $(w_1, w_2) \geq 0$ the sets \eqref{eq:indexsets} define
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
The regularization $t$ plays the role of the Scholtes relaxation parameter (see \S\ref{sec:mpcc:solution})
...
from the Challenge 1 of the GO competition\footnote{\url{https://data.openei.org/submissions/6153}}.
...
We detail the results in Table~\ref{tab:scopfscalability}. In addition to the performance we obtain with ExaModels on the CPU and on the GPU,
...
the number of contingencies for Knitro and MadNCL (running on the CPU with JuMP, SymbolicAD and ExaModels
...
\includegraphics[width=.4\textwidth]{./figures/scalability_decomposed.pdf}
...
We emphasize that apart from the contingency screening used in \S\ref{sec:numerics:screening},
our algorithm solves the problem \eqref{eq:scopf} frontally, without using any heuristic or structure exploiting method.
On this new benchmark, we observe that MadNCL-GPU is consistently faster than Knitro. As in Table~\ref{tab:scopfscalability},
